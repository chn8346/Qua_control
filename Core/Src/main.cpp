...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
    ...
    HAL_UART_Receive_IT(&huart1,(uint8_t*)u1rx_buff,1); // 开 uart1 中断
    HAL_UART_Receive_IT(&huart2,(uint8_t*)u2rx_buff,1); // 开 uart2 中断
    LED1_ON;    // 关闭 LED1
    LED2_ON;    // 关闭 LED2
  ...
  while (1)
  {
    ...
    // i2c_scan_print(&hi2c1, 0x75, ICM_ID_KEY); // ICM get addr
    // i2c_scan_print(&hi2c2, SPL06_ID_ADDR, SPL06_ID_KEY); // SPL get addr
    // i2c_scan_print(&hi2c2, IST8310_ID_ADDR, IST8310_ID_KEY); // IST8310 get addr
    if(send_sgn != 0) {
        send_sgn = 0;
        cnt++;
        ...
        if(cnt >= cnt_limit)
        {
            ...
            char msg[150] = {0};
            float quat[4] = {q0x, q1x, q2x, q3x};

            float pitch = asin(2*(q0x*q2x-q1x*q3x))*rd;
            float roll = atan(2*(q0x*q1x+q2x*q3x)/(1-2*(q1x*q1x+q2x*q2x)))*rd;
            // float yaw = atan(2*(q0x*q3x+q1x*q2x)/(1-2*(q2x*q2x+q3x*q3x)))*rd;
            float yaw = atan2(2*(q0x*q3x+q1x*q2x),(1-2*(q2x*q2x+q3x*q3x)))*rd;
            //float pitch = 0.0;
            //float roll = 0.0;
            //float yaw = 0.0;
            // calibrate_return_data(datap, datap+3, mag_fdata);

#if 1
            sprintf(msg, "pitch=%f,roll=%f,yaw=%f,",pitch, roll, yaw);
#elif 0
...
static void MX_TIM1_Init(void)
{
  ...
  htim1.Init.Period = 100; // 10000 刚好为1s， 1ms +10
