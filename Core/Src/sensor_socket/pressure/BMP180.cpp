...
void BMP180_get_temperature(float* tmp){
    uint8_t value;
    int iv = 0;
    ...
    *tmp = (101325 - *tmp)/133*12;
}

void BMP180_get_pressure(float* psr){
    uint8_t value = BMP180_CONTROL_KEY;
    HAL_I2C_Mem_Write(i2c_bmp, BMP180_ADDR_WRITE, BMP180_CONTROL_REG, 1, &value, 1, 500);
...
//    iv = value>>3;  // 高四位可用
    ...
    iv = (iv << 8) + value;
//    HAL_I2C_Mem_Read(i2c_bmp, BMP180_ADDR_READ, BMP180_XLSB, 1, &value, 1, 500);
//    iv = (iv << 8) + (value>>3);  // 高四位可用
   *psr = (float)iv;
  // *psr = 6700-(101325 - *psr)/133*12;

}

// TODO 【未完成】 收尾工作还没有完成，需要等传感器到了之后测一下
void BMP180_get_height(float* h){
    *h = 0;
}
