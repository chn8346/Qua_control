...
void crc16(uint8_t * data,uint8_t data_len_pcs,uint8_t* ret_crc)
{
    ...
    while(data_len_pcs--)/*传输消息缓冲区*/
    { uIndex=uchCRCHi^(*(data++));/*计算 CRC*/
        uchCRCHi=uchCRCLo^auchCRCHi[uIndex];
        uchCRCLo=auchCRCLo[uIndex];
    }

    // uint16_t r = ((uchCRCHi<<8)|uchCRCLo);
    ret_crc[0] = uchCRCLo;
    ret_crc[1] = uchCRCHi;
}

void crc16c(unsigned char* a, unsigned char b, unsigned char* c)
{
    unsigned char uchCRCHi = 0xFF;/*高 CRC 字节初始化*/
    unsigned char uchCRCLo = 0xFF;/*低 CRC 字节初始化*/
    unsigned uIndex;/*CRC 循环中的索引*/
    while(b--)/*传输消息缓冲区*/
    { uIndex=uchCRCHi^(*(a++));/*计算 CRC*/
        uchCRCHi=uchCRCLo^auchCRCHi[uIndex];
        uchCRCLo=auchCRCLo[uIndex];
    }
